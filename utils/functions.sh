...
handle_standard_options ()
{
	if ! $found ; then
		...
		elif [[ $1 == '--gtk4'           ]] ; then gtk_type=gtk4
		else
...
#		which python$pyver-config > /dev/null
		...
		LDLIBS="$LDLIBS $(python$pyver-config --libs ) -L$(find /usr/lib/python$pyver -name libpython$python_version.a -printf %h)"
	fi
}

detect_gtk ()
{
	if [[ $gtk_type == 'gtk4' ]] ; then gtk_pkg=gtk+-4.0 ; vte_pkg=vte-2.91
	else                                gtk_pkg=gtk+-3.0 ; vte_pkg=vte-2.91 ; fi
